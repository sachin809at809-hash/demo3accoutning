<ul class="pagination justify-content-end mb-0">
    ...
    @foreach ($elements as $element)
        ...
        @if (is_string($element))
            <li class="font-medium ltr:mr-4 rtl:ml-4 disabled"><a class="page-link" href="#" tabindex="-1">{{ $element }}</a></li>
        @endif
        ...
        @if (is_array($element))
            @foreach ($element as $page => $url)
                @if ($page == $paginator->currentPage())
                    <li class="font-medium ltr:mr-4 rtl:ml-4 active"><a class="page-link" href="#" tabindex="-1">{{ $page }}</a></li>
                @else
                    <li class="font-medium ltr:mr-4 rtl:ml-4"><a class="page-link" tabindex="-1" href="{{ $url }}">{{ $page }}</a></li>
                @endif
            @endforeach
        @endif
    @endforeach
